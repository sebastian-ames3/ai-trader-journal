...
| 5 - Mobile | In Progress | PWA enhancement (complete), Capacitor migration (pending), App Store (pending) |

### Phase 5 Progress
- [x] iOS meta tags & splash screens (6 device sizes)
- [x] Enhanced manifest.json with screenshots & shortcuts
- [x] Offline fallback page (`/offline`)
- [x] API route caching (theses, coach, insights, share)
- [x] Next.js 15 async params migration (26+ routes)
- [x] Cache module for market data (`src/lib/cache.ts`)
- [ ] Generate actual splash screen images
- [ ] Generate App Store screenshots via Playwright
...
2. Legacy files with TS warnings: `src/lib/yahooFinance.ts`, `src/lib/polygonClient.ts`
3. yfinance service needs Railway deployment for production
4. VAPID keys, pgvector not yet configured
5. PWA splash/screenshots need actual images (placeholders configured)

## Media Upload Infrastructure

**Storage:** Cloudflare R2 (configured) | **Transcription:** OpenAI Whisper | **Analysis:** GPT-4o-mini

| Feature | Endpoint | Limits |
|---------|----------|--------|
| Voice Recording | `/api/upload/audio` + `/api/transcribe` | 10MB, 5 min |
| Image Upload | `/api/upload/image` + `/api/analyze/image` | 5MB, 5 images |
| Quick Capture | `/api/infer` (Claude Haiku) | Auto-inference |

**Components:** `VoiceRecorder`, `ImageCapture`, `QuickCapture`, `AudioPlayer`

## PWA Configuration

**Manifest:** `public/manifest.json` - App name, icons, screenshots, shortcuts
**Service Worker:** Generated by `next-pwa` in `public/sw.js`
**Offline Fallback:** `/offline` page with cached content info

### iOS Support
- Splash screens for 6 device sizes (iPhone 14 Pro Max down to SE, iPad Pro)
...
| Route | Strategy | TTL |
|-------|----------|-----|
| `/api/entries` | NetworkFirst | 5 min |
| `/api/theses` | NetworkFirst | 5 min |
| `/api/coach` | NetworkFirst | 5 min |
| `/api/insights` | NetworkFirst | 10 min |
| Static assets | StaleWhileRevalidate | 24 hr |
| Fonts | CacheFirst | 365 days |

### Asset Generation
```bash
node scripts/generate-pwa-assets.js  # Requires dev server running
```
...
| Insights | `/insights` |
| Pattern Insights | `/insights/patterns` |
| Shared Entry | `/share/[slug]` |
| Offline | `/offline` |
